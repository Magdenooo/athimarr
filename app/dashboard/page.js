...
const ProgressBar = ({ progress, color = "blue" }) => (
  <div className="w-full bg-gray-200 rounded-full h-4 mb-2">
    <div 
      className={`bg-${color}-600 h-4 rounded-full transition-all duration-500 ease-out`} 
      style={{ width: `${progress}%` }}
    ></div>
  </div>
);

const FeatureSection = ({ title, children, icon: Icon, color = "blue" }) => (
  <div className="mt-8">
...
const BookSection = () => (
  <FeatureSection title="المكتبة" icon={Book} color="indigo">
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {[
        { title: 'عادات العظماء', author: 'جون دو', color: 'blue', rating: 4.5, progress: 75 },
        { title: 'قوة العادات', author: 'تشارلز دوهيج', color: 'green', rating: 4.8, progress: 30 },
        { title: 'العقل المرن', author: 'كارول دويك', color: 'purple', rating: 4.2, progress: 100 },
      ].map((book, index) => (
    ...
    <button className="mt-6 bg-indigo-500 text-white px-6 py-2 rounded-full hover:bg-indigo-600 transition duration-300 w-full">
...
const QuoteSection = () => (
  <FeatureSection title="اقتباسات ملهمة" icon={Award} color="yellow">
    <div className="space-y-4">
      {[
        { text: 'النجاح هو مجموع جهود صغيرة تتكرر يومًا بعد يوم.', author: 'روبرت كولير', color: 'blue' },
        { text: 'الفشل هو ببساطة فرصة للبدء من جديد، هذه المرة بذكاء أكثر.', author: 'هنري فورد', color: 'green' },
      ].map((quote, index) => (
        <blockquote key={index} className={`border-r-4 border-${quote.color}-500 pr-4 py-2 bg-${quote.color}-50 rounded-lg`}>
          <p className="italic text-lg">"{quote.text}"</p>
          <footer className="text-right mt-2 text-gray-600">- {quote.author}</footer>
        </blockquote>
      ))}
    </div>
    <button className="mt-6 bg-yellow-500 text-white px-6 py-2 rounded-full hover:bg-yellow-600 transition duration-300 w-full">
...
function Page() {
  ...
  const tabContent = {
    ...
    quotes: <QuoteSection />,
    achievements: (
      <FeatureSection title="آخر الإنجازات" icon={Award} color="orange">
    ...
    <div className="flex bg-gray-100 min-h-screen" dir="rtl">
      ...
      <main className="flex-grow p-8">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          <h2 className="text-2xl font-bold mb-4 text-indigo-700">تقدم التعافي</h2>
